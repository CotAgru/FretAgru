...
-- ORDEM CORRETA: Criar tabelas auxiliares ANTES das foreign keys

-- PASSO 1: Criar tabela tipos_contrato
...
-- PASSO 2: Criar tabela unidades_medida
...
-- PASSO 3: Adicionar campo ano em safras
ALTER TABLE safras 
ADD COLUMN IF NOT EXISTS ano INTEGER;

COMMENT ON COLUMN safras.ano IS 'Ano da safra (ex: 2024, 2025)';

-- Atualizar safras existentes
UPDATE safras 
SET ano = CASE 
...
-- PASSO 4: Adicionar campos ano_safra (sem FK)
...
-- PASSO 5: Adicionar campos com FK (tabelas já existem agora)
ALTER TABLE contratos_venda 
...
-- PASSO 6: Renomear e adicionar campos em contratos_venda
DO $$ 
BEGIN
  -- Renomear volume_tons para quantidade (se existir)
  IF EXISTS (SELECT 1 FROM information_schema.columns 
             WHERE table_name='contratos_venda' AND column_name='volume_tons') THEN
    ALTER TABLE contratos_venda RENAME COLUMN volume_tons TO quantidade;
  END IF;
  
  -- Renomear preco_valor para valor_unitario (se existir)
  IF EXISTS (SELECT 1 FROM information_schema.columns 
             WHERE table_name='contratos_venda' AND column_name='preco_valor') THEN
    ALTER TABLE contratos_venda RENAME COLUMN preco_valor TO valor_unitario;
  END IF;
END $$;
...
-- PASSO 7: Adicionar campos em contratos_compra_insumo
DO $$ 
BEGIN
  -- Renomear preco_valor para valor_unitario (se existir)
  IF EXISTS (SELECT 1 FROM information_schema.columns 
             WHERE table_name='contratos_compra_insumo' AND column_name='preco_valor') THEN
    ALTER TABLE contratos_compra_insumo RENAME COLUMN preco_valor TO valor_unitario;
  END IF;
END $$;
...
COMMENT ON COLUMN contratos_compra_insumo.unidade_medida_id IS 'Unidade de medida (kg, tn, lt, etc)';
COMMENT ON COLUMN contratos_compra_insumo.valor_unitario IS 'Valor unitário conforme unidade';
COMMENT ON COLUMN contratos_compra_insumo.valor_total IS 'Valor total calculado (quantidade * valor_unitario)';
